      ...
      <main style={{ maxWidth: 1280, margin: "0 auto", padding: "28px 32px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 12, marginBottom: 24 }}>
          {[
            ...
            { label: "Applied",      value: String(data.total_applied), sub: "Total applications", color: "#2563eb" },
            ...
            { label: "Job Matches",  value: String(data.top_job_matches?.length ?? 0), sub: "AI matched roles", color: "#8b5cf6" },
            { label: "Profile",      value: `${profile.score}%`, sub: "Completeness", color: profile.score >= 80 ? "#10b981" : profile.score >= 50 ? "#f59e0b" : "#ef4444" },
          ].map((m) => (
            <div key={m.label} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 14, padding: "18px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr", gap: 18 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {/* Profile card */}
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "20px", textAlign: "center" }}>
            ...
            <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "18px" }}>
              ...
              {[
                ...
                <Link key={href} href={href} style={{ display: "block", padding: "8px 10px", borderRadius: 8, fontSize: 13, color: "#374151", textDecoration: "none", marginBottom: 2, transition: "background 0.15s" }}
                    ...
                    return (
                      <div key={stage} style={{ display: "flex", alignItems: "center", flex: 1 }}>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
                          <div style={{ width: isCurrent ? 14 : 10, height: isCurrent ? 14 : 10, borderRadius: "50%", background: isCurrent ? "#2563eb" : isPast ? "#bfdbfe" : "#e2e8f0", border: `2px solid ${isCurrent ? "#2563eb" : isPast ? "#93c5fd" : "#e2e8f0"}`, boxShadow: isCurrent ? "0 0 0 4px #dbeafe" : "none", transition: "all 0.3s" }} />
                          <span style={{ fontSize: 8, color: isCurrent ? "#2563eb" : isPast ? "#93c5fd" : "#cbd5e1", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: isCurrent ? 700 : 400 }}>
                            {STAGE_LABELS[stage]}
                          </span>
                        </div>
              ...
              <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "22px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  {data.top_job_matches.slice(0, 5).map((job: any, i: number) => (
                    <Link key={i} href={`/jobs/${job.id}`} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 10px", borderRadius: 10, textDecoration: "none", transition: "background 0.15s" }}
                      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "#f8fafc"; }}
                      onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = "transparent"; }}>
                      <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {topApp?.benchmark_score != null && (
              <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "20px", textAlign: "center" }}>
                ...
                <p style={{ fontSize: 36, fontWeight: 700, color: "#2563eb", letterSpacing: "-0.03em" }}>
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
          {[0, 1, 2].map((i) => (
            <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: "#2563eb", opacity: 0.4, animation: "bounce 1s infinite", animationDelay: `${i * 0.15}s` }} />
          ))}
        </div>
      </div>
    </div>
