...
FIXES:
- BUG 1: total score floored at 0 via max(0, ...) — prevents StopIteration
         if breakdown values somehow sum negative.
- BUG 3: no-JD keyword fallback now sets is_generic_density=True so the
         frontend knows it's not a real JD match score.
- RISK 2: is_fallback flag propagated from scorer into the final report.
- RISK 4: removed the 100-line commented-out duplicate class.
"""
...
class ATSReport:
    ...
    is_fallback:      bool       # True when Groq failed and defaults were used
...
class ATSReportGenerator:
    ...
    def generate(
        ...
        if jd_text:
            ...
            # FIX BUG 3: flag this clearly so frontend doesn't show it as a JD match
            keyword_result = {
                "matched":            [],
                "missing":            [],
                "score":              (llm_scores or {}).get("keyword_density", 20),
                "coverage":           0.0,
                "is_generic_density": True,
            }
        ...
        improvement_tips = self._build_tips(breakdown, section_result, keyword_result)

        # FIX RISK 2: surface whether LLM scores are real or fallback defaults
        is_fallback = bool((llm_scores or {}).get("is_fallback", False))
